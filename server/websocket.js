...
const colorMap = { // map of code IDs to colors
    0: 'blue',
    1: 'green',
    2: 'yellow'
}
...
function getPlayerList(session) {
    return Object.keys(session.players).map(username => {
        const { codeId } = session.players[username]
        return {
            username,
            codeId,
...
function getAvailablePlayerCodeId(session) {
    const usedCodeIds = new Set(Object.values(session.players).map(player => player.codeId))
    ...
    if (isSpectator) {
        ...
        ws.on('message', message => {
            ...
            if (type === 'hit') {
                ...
                console.info(
                    "THIS IS WHAT IS BEING BROADCASTED TO ALL USERS" + spectatorMessage
                );
...
function handleHit(session, player, codeId, weapon) {
    if (codeId >= 0 && codeId <= 9) { // player hit
